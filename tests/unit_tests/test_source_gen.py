...
def test_source_gen():
    for l in langs_to_test:
        if(l=='zh'):
            result = validator._generate_source("你好世界", lang=l)
            assert type(result) == str
            token_output = validator._wenzhong_gpt2_pipeline.tokenizer.encode(result)
        elif(l=='en'):
            result = validator._generate_source("hello world", lang = l)
            assert type(result) == str
...
def test_generate_source():
    for l in langs_to_test:
        ...
        prompts = []
        for text in texts:
            prompts.append(validator._generate_source(text, lang = l))
